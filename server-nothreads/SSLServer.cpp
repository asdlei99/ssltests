...
bool SSLServer::init() {
	...
	char inter[8];
	sprintf(inter, "*:%i", SERVER_PORT); // Format: host:port (* = any)
	BIO_set_accept_port(listenBIO, inter);

	// Bind (b/c it's non blocking, we need to keep retrying the bind until the bind is successful)
	if(BIO_do_accept(listenBIO) <= 0) {
		while(BIO_should_retry(listenBIO)) {
			if(BIO_do_accept(listenBIO) > 0) {
				break;
			}
		}
		if(!BIO_should_retry(listenBIO) && BIO_do_accept(listenBIO) <= 0) {
